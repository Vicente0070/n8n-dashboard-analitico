...
export const ExecutionLogsTable = ({ execs, workflows, n8nUrl }: ExecutionLogsTableProps) => {
  ...
  const filteredExecs = selectedWorkflow === "all" 
    ? execs 
    : execs.filter((exec) => exec.workflowId === selectedWorkflow);

  const displayedExecs = filteredExecs.slice(0, displayCount);

  const workflowsWithExecs = workflows.filter((w) => 
    execs.some((e) => e.workflowId === w.id)
  );

  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:gap-0 sm:flex-row sm:items-center justify-between">
        <h2 className="section-title">Logs de Execução</h2>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-1 sm:flex-none">
            ...
            <Select value={selectedWorkflow} onValueChange={setSelectedWorkflow}>
              ...
              <SelectContent className="bg-card border-border">
                ...
                {workflowsWithExecs.map((workflow) => (
      ...
      <div className="md:hidden space-y-2 max-h-[400px] overflow-y-auto">
        {displayedExecs.map((exec) => (
          <div
            ...
            className="mobile-log-card active:bg-secondary"
          >
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-muted-foreground text-[10px] terminal-text font-mono">
                  #{exec.id}
                </p>
              </div>
              <div className="flex items-center gap-1.5">
                <span className={`status-dot ${exec.status === "success" ? "status-dot-success" : "status-dot-error"}`} />
            ...
            <tbody className="divide-y divide-border">
              {displayedExecs.map((exec) => (
                <tr
                  key={exec.id}
                  onClick={() => openExecution(exec.workflowId, exec.id)}
                  className="hover:bg-secondary/50 cursor-pointer fast-transition terminal-text"
                >
                  <td className="px-4 py-2.5 text-muted-foreground font-mono text-xs">{exec.id}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      <span className={`status-dot ${exec.status === "success" ? "status-dot-success" : "status-dot-error"}`} />
                      <span className={`text-xs ${exec.status === "success" ? "text-primary" : "text-destructive"}`}>
